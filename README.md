...
### Release Notes
- **2026.07** added "Locally Meshable Frame Fields", "Stress-Aligned Hexahedral Lattice Structures", "HybridOctree_Hex - Hybrid Octree-Based Adaptive All-Hexahedral Mesh Generation with Jacobian Control" and "HexOpt - Efficient and Robust Hexahedral Mesh Optimization", bringing the collection to 1300 meshes from 32 papers. Large meshes can now be stored gzip-compressed (`.vtk.gz` / `.mesh.gz`) and are transparently decompressed on load, and the VTK loader now accepts any legacy version header (2.0 through 5.x). The snapshot button can now save high-resolution PNGs: Alt+click it to cycle a 2x / 4x / 8x render-resolution factor (shown as a small badge on the button), then click normally to save at that size (capped by the browser/GPU canvas limit). Upgraded the rendering engine from three.js r145 to r185.
